    ...
    for (let [entry, locations] of Object.entries(userData)) {
        ...
        if (entry.startsWith('_')) {
            ...
            if (entry.toLowerCase() === 'khonsunotes') { 
                ...
                $khonsuNotes.innerHTML = locations.replaceAll('-','<br>-').replace(/^\n/,''); 
            }
                ...
                $reservation.querySelectorAll(`div:nth-child(${2}) ~ *`).forEach(cell => cell.remove());
...
function populateDays($daySlide, dayNum, daysDate, num, title, dayEventName) {
    dayNum === '1' ? populateDay1($daySlide, dayNum, daysDate, num, title, dayEventName) : populateMoreDays($daySlide, dayNum, daysDate, num, title, dayEventName); 
}    
...
function populateMoreDays($daySlide, dayNum, daysDate, num, title, dayEventName) {
    // const $daySlide = document.querySelector('.cs_slide.is-8');
    const $daySlideClone = $daySlide.cloneNode(true);
