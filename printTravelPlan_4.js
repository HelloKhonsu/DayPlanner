    ...
    for (let [day, locations] of Object.entries(userData)) {
        ...
        locations.forEach((location, num) => {
            ...
            if (lat && lng) {
                ...
                populateDays($daySlide, $daySlideClone, dayNum, num, title, dayEventName); 

                console.log(`Location ${num}:`, location)
            }
        });
...
function populateDays($daySlide, $daySlideClone, dayNum, num, title, dayEventName) {
    dayNum === '1' ? populateDay1($daySlide, dayNum, num, title, dayEventName) : populateMoreDays($daySlide, $daySlideClone, dayNum, num, title, dayEventName); 
}    

function populateDay1($daySlide, dayNum, num, title, dayEventName) {
    // const $daySlide = document.querySelector('.cs_slide.is-8');
    setupSlide($daySlide, dayNum, num, title, dayEventName); 
} 

function populateMoreDays($daySlide, $daySlideClone, dayNum, num, title, dayEventName) {
    // const $daySlide = document.querySelector('.cs_slide.is-8');
    // const $daySlideClone = $daySlide.cloneNode(true);
