...
function postDayEvent(dayEventTxt, day, marker, eventId, markerObj) {
    ...
    if ($day) {
        ...
        if ($dayEvents.querySelector(`.day-${dayNum}-event`)) {
            ...
            console.log('single-event kwa function postDayEvent', $hiddenEvent)
...
function constructEvent(dayEventTxt, day, marker, eventId, markerObj) {
    ...
    console.log(
        'single-event kwa constructEvent', $dayEvents.querySelector(`${day} > .single-event`)   
    )
...
$dayEvents.addEventListener('click', e => {
    ...
    // const $modal = e.target.closest('.map_sort-button').querySelector('.pop-up');
    const $modal = $dayEvent.querySelector('.pop-up');
    ...
    $allEvents.querySelectorAll('.single-event').forEach(dayEvent => {
        const $popupSingleEventClone = $modalContent.querySelector('.single-event').cloneNode(true); 
        $popupSingleEventClone.querySelector('.day-text').textContent = dayEvent.querySelector('.day-text').textContent;
        if (dayEvent.marker) $popupSingleEventClone.marker = dayEvent.marker;
        $popupSingleEventClone.markerObj = dayEvent.markerObj;
        $modalContent.append($popupSingleEventClone);
    });
    ...
    $close.addEventListener('click', e => {
        ...
        const $sortedEvents = $closeBtn.closest('.pop-up_item').querySelectorAll('.all-events .single-event');

        const $newEventList = replacePriorEventsWithSortedEvents($sortedEvents, $allEvents); 

        $allEvents.replaceWith($newEventList);
    }); 

    $modal.addEventListener('click', e => { 
        if (e.target !== $modal) return;    
        // const $sortedEvents = e.target.querySelector('.all-events');
        const $sortedEvents = e.target.querySelectorAll('.all-events .single-event');
...
*/
function replacePriorEventsWithSortedEvents($sortedEvents, $allEvents) {
    const $newEventList = document.createElement('div');
    $newEventList.className = 'all-events'; 
    $sortedEvents.forEach(dayEvent => {
        const $singleEventClone = $allEvents.querySelector('.single-event').cloneNode(true); 
        $singleEventClone.querySelector('.day-text').replaceWith(dayEvent.querySelector('.day-text'));
        $singleEventClone.markerObj = dayEvent.markerObj;
        if (dayEvent.marker) $singleEventClone.marker = dayEvent.marker;  
        $newEventList.append($singleEventClone); 
    });
    return $newEventList; 
}
...
async function retrieveSavedMarkersFromFirebase(userMail) {
    ...
    querySnapshot.forEach((doc) => {
        ...
        console.log( 'single-event kwa retrieval', $dayEvents.querySelector(`${day} > .single-event`) )

        const savedMarkers = doc.data();
        if (savedMarkers.eventsArr && savedMarkers.eventsArr.length) { 
            savedMarkers.eventsArr.forEach((marker, eventNum) => {
                const {lat, lng, title, dayEventName} = marker; 
                const createdMarker = createMarker(title, {lat, lng});   
                currentDay.markers.push(createdMarker);  
                postDayEvent(dayEventName, day, createdMarker, `event${(eventNum+2)}-day${dayNum}`, {lat, lng, title, dayEventName}); 
            });
        }
        else {
...
async function updateFirebaseAfterSort($dayEvent) {
    ...
    const existingMarkers = doc(db, `Markers-${userMail}`, `day${dayNum}`); 
    await setDoc(existingMarkers, { eventsArr }); 
}
